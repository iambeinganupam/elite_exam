...
if(isset($_GET['examid'])){
	...
	if($fetch_exam_details_query){
		...
		<html>
		    ...
		    <body>
		    	...
		    	while($fetch_row = mysqli_fetch_assoc($fetch_exam_details_query)){
		    		...
		    		$get_students = "SELECT * FROM students_$lower_examname";
		    		$get_students_query = mysqli_query($org_con2, $get_students);
		    		?>
		...
		<tbody>
			<tr>
				<?php
				while($student_row = mysqli_fetch_assoc($get_students_query)){
				?>
				<td><?php echo $student_row['serial_no']; ?></td>
				<td><?php echo $student_row['student_name']; ?></td>
				<td><?php echo $student_row['student_id']; ?></td>
				<td><?php echo $student_row['student_password']; ?></td>
				<td>
					<?php
					if($student_row['student_name'] == ""){
						?>
						<a href="exam/activate_exam.php?examid=<?php echo $exam_row['exam_id']; ?>" class="btn btn-success">Activate</a>
						<?php
					}else{
						?>
						<a href="#" class="btn btn-secondary">Activated</a>
						<?php
					}
					?>
				</td>

				<td><a href="exam/delete_exam.php?examid=<?php echo $student_row['exam_id']; ?>" class="btn btn-danger">Delete</a></td>
			</tr>
			<?php
				}
			?>
